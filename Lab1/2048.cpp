...
int resume = 0; // resume from previous record
/**
 * output streams
...
protected:
	...
	size_t indexof(const std::vector<int>& patt, const board& b) const {
		...
		size_t index = 0;
		for (int t : patt) {
			index = (index << 4) | b.at(t); // equivalent to index = index * 16 + b.at(t)
		}
		return index;
	}
...
public:
			...
			if (move->assign(b)) {
				...
				for (int i=0; i<16 ; i++){
					if (after_state.at(i) == 0){
						after_state.set(i, 1);
						total_value += 0.9 * estimate(after_state); //P(2^1) = 0.9 
						after_state.set(i, 2);
						total_value += 0.1 * estimate(after_state); //P(2^2) = 0.1
						after_state.set(i, 0);						
						value_count += 1;
					}
				}
				move->set_value(move->reward() + total_value / (std::max(value_count, 1))); //avoid divide by zero 
	...
	void update_episode(std::vector<state>& path, float alpha = 0.1) const {
		...
		for (auto it = path.rbegin(); it != path.rend(); it++) {
			error = target - estimate(it->before_state());
	 ...
	 */
	void make_statistic(size_t n, const board& b, int score, int unit, std::ofstream& f, std::chrono::time_point<std::chrono::_V2::system_clock, std::chrono::duration<long int, std::ratio<1, 1000000000> > > start_time, int total) {
		...
		if (n % unit == 0) { // show the training process
			...
			double eta_seconds = (elapsed_seconds / (n - resume)) * (total - n);

			// Calculate hours and minutes for elapsed time
			int elapsed_hours = static_cast<int>(elapsed_seconds / 3600);
			int elapsed_minutes = static_cast<int>((elapsed_seconds - elapsed_hours * 3600) / 60);

			// Calculate hours and minutes for ETA
			int eta_hours = static_cast<int>(eta_seconds / 3600);
			int eta_minutes = static_cast<int>((eta_seconds - eta_hours * 3600) / 60);

			// Print elapsed time and ETA every unit episodes
			info << "Episode " << n/1000 << "k/" << total/1000 << "k - Time: " 
				<< elapsed_hours << " hours " << elapsed_minutes << " minutes - ETA: " 
				<< eta_hours << " hours " << eta_minutes << " minutes" << std::endl;
		}
	}
...
int main(int argc, const char* argv[]) {
	...
	// create csv file for recording the training process
	std::ofstream csv = std::ofstream("record.csv", std::ios::out | std::ios::trunc);
	csv << "episode,score,maxtile" << std::endl;

	// check if the file is opened
	if (!csv.is_open()) {
		error << "failed to open record.csv" << std::endl;
		std::exit(1);
	}
	...
	for (size_t n = 1; n <= total; n++) {
		...
		tdl.make_statistic(n, b, score, 1000, csv, start_time, total);
		path.clear();
	}
